    ...
    if (rects) {
      ...
      if (!streaming) {
        ...
        page.clipRect = {
          top: options.shotOffset.top
          , left: options.shotOffset.left
          , width: pixelCount(page, 'width', 100)
          - options.shotOffset.right
          , height: pixelCount(page, 'height', 100)
          - options.shotOffset.bottom
        };
        page.render("flickr2.jpeg", {quality: options.quality});
...
if (options.onCallback && options.takeShotOnCallback) {
  takeScreenshot = function(data) {
    ...
    if (data.action == 'takeShot') {
      console.log("Rects: " + JSON.stringify(data.data));
      _takeScreenshot(data.data);
    }
  };
...
if (options.siteType == 'url') {
  var postParams = options.shotOffset.credentials;
  ...
  if (options.takeShotOnCallback) {
    ...
    console.log(site, JSON.stringify(postParams));
    page.open(site, 'post', postParams, takeScreenshot);
  }
} else {
